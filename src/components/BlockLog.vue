<template>
  <div>
    <el-dialog :visible.sync="axeLogDetailVisible" title="Block by Instruments">
      <el-form v-if="selectedLog" class="log-detail" label-width="120px" label-position="left">
        ...
        <el-form-item label="cost" style="margin-top: -15px;">
          <div>{{ selectedLog.size + ' 毫秒' }}</div>
        </el-form-item>
      </el-form>
    </el-dialog>
    <div class="lane leak">
      <el-popover v-for="item in blockList" :key="item.id" placement="top-start" title="Block"
        width="160" trigger="hover" :content="item.name">
        <div class="point" slot="reference" @mouseover="mouseover(item)" @mouseout="mouseout(item)" @click="showDetailLog(item)">B</div>
      </el-popover>
    </div>
  </div>
</template>
